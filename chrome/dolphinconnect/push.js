...
function push(id, msg, flag, type, subtype) {
    ...
    if (type == 3) {
        if (getByteLength(msg.title) > 1000 * 2) {
            chrome.tabs.executeScript(null, {
                code: INJECT_CODE.SEND_FAIL
            });
            chrome.tabs.executeScript(null, {
                file: "cs_overlayer.js"
            });
            return;
        }
    } else {
    ...
    {
        chrome.tabs.executeScript(null, {
            code: INJECT_CODE.NETWORK_ERROR
        });
        chrome.tabs.executeScript(null, {
            file: "cs_overlayer.js"
        });
...
chrome.extension.onMessage.addListener(function(message, sender, sendResponse) {
    ...
    if (type == 'map') {
        ...
        if (!setting['button']) {
            sendResponse({
                status: 2
            }); // 2 stands for button not be enabled.
            return;
        }
